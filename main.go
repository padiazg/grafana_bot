package main // import "https://github.com/padiazg/grafana_bot"
...
func str_Format_Byte(in float64, j1 int) string {
	var str_Size string
	f := in
...
func str_FormatFloat(f float64) string {
...
var funcMap = template.FuncMap{
	...
	// "str_Byte_To_Kilobyte":   str_Byte_To_Kilobyte,
	"HasKey":                 HasKey,
}
...
func AlertFormatStandard(alerts Alerts) string {
	matches := make([]string, len(alerts.EvalMatches))
	for i, a := range alerts.EvalMatches {
		matches[i] = fmt.Sprintf("\n%s: (%v)", a.Metric, a.Value);
	}
	return fmt.Sprintf(
		"<a href='%s'>%s</a>\n%s \nMatches: %s",
		alerts.RuleUrl,
